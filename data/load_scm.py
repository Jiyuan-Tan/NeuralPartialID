...
class SCM:
    ...
    def generate(self, n_samples: int, ratio: float = 0) -> np.ndarray:
        if self.name == 'nonlinear_padh_iv_modified':
            data = self.generate_func(n_samples, ratio)
        else:
...
def backdoor_counterexample():
    def generate(n_samples: int) -> np.ndarray:
        x = np.random.binomial(1, 0.5, size=(n_samples, 1)) - 1
        t = np.zeros(n_samples)
        y = np.zeros(n_samples)
        for i in range(n_samples):
            if x[i] == 0:
                t[i] = np.random.binomial(1, 5/8, size=(1, 1))
                if t[i] == 1:
                    y[i] = np.random.uniform(-1, 1, size=(1,1)) + np.random.binomial(1, 3/5, size=(1, 1))
                else:
                    y[i] = np.random.uniform(-1, 1, size=(1,1)) + np.random.binomial(1, 1/3, size=(1, 1))
            else:
                t[i] = np.random.binomial(1, 1/2, size=(1, 1))
                y[i] = np.random.uniform(-1, 1, size=(1,1))  + t[i]
    ...
    dag = gen_dags('backdoor', var_dims=np.array([1, 1, 1]), binary_keys=[1])
    scm = SCM('backdoor_counterexample', dag, generate)
    scm.set_estimand('ATE', 19/30)
    return scm


def backdoor_counterexample_binary():
    def generate(n_samples: int) -> np.ndarray:
        x = np.zeros(n_samples)
        tmp = np.random.binomial(1, 0.5, size=(n_samples, 1))
        t = np.array([])
        y = np.array([])
        for i in tmp:
            if i == 0:
                t = np.append(t, np.random.binomial(1, 0.5, size=(1, 1)))
                y = np.append(y, np.random.uniform(-1, 1, size=(1,1)))
            else:
                t = np.append(t, np.random.binomial(1, 0.75, size=(1, 1)))
                y = np.append(y, np.random.uniform(0, 2, size=(1,1)))
        # print(t.shape, y.shape, x.shape)
        x = x.reshape(-1, 1)
        y = y.reshape(-1, 1)
        t = t.reshape(-1, 1)
    ...
    dag = gen_dags('backdoor', var_dims=np.array([1, 1, 1]), binary_keys=[1])
    scm = SCM('backdoor_counterexample_binary', dag, generate)
    scm.set_estimand('ATE', 3/5-1/3)
    return scm
...
def binary_iv():
    ...
    def generate(n_samples: int) -> np.ndarray:
        ...
        data = torch.cat((tmp[:, 0].reshape(-1, 1), tmp[:,1].reshape(-1, 1), tmp[:, 2].reshape(-1, 1)), dim=1).float()
        return data.numpy()

    dag = gen_dags('iv', var_dims=np.array([1, 1, 1]), binary_keys=[0, 1, 2])
...
def binary_iv_mono():
    strata_xy = np.asarray([
        ...
        [0.000757, 0.013034, 0.006125, 0.002606],  # x00
        [0.0, 0, 0.0, 0.0],  # x10
        [0.030581, 0.492952, 0.229945, 0.096651],  # x01
        [0.004534, 0.073950, 0.034123, 0.014742]  # x11
    ])
    ...
    dag = gen_dags('iv', var_dims=np.array([1, 1, 1]), binary_keys=[0, 1, 2], monotonic_index ={1:[0]})
    scm = SCM('binary_iv_monotonic', dag, generate)
    return scm
...
def nonlinear_padh_iv_modified():
    def generate(n_samples: int, ratio: float = 0) -> np.ndarray:
        e_y = ratio * np.array(truncnorm.rvs(-1, 1, size = (n_samples,1))) + (1 - ratio) * np.random.uniform(-1, 1, size=(n_samples, 1))
        c = ratio * np.array(truncnorm.rvs(-1, 1, size = (n_samples,1))) + (1 - ratio) * np.random.uniform(-1, 1, size=(n_samples, 1))
        z = ratio * np.array(truncnorm.rvs(-1, 1, size = (n_samples,1))) + (1 - ratio) * np.random.uniform(-1, 1, size=(n_samples, 1))
        t = (1.5 + z + 0.5 * c)/3
        for i in range(n_samples):
            t[i] = np.random.binomial(1, t[i], size = (1,1))
        y = 3 * t - 1.5 * t * c + c + e_y
        data = np.concatenate([z, t, y], axis=-1)
        return data

    dag = gen_dags('iv', var_dims=np.array([1, 1, 1]), binary_keys=[1])
    scm = SCM('nonlinear_padh_iv_modified', dag, generate)
    scm.set_estimand('ATE', 3)
    return scm
...
def leaky():
    def generate(n_samples: int) -> np.ndarray:
        u = np.random.uniform(-1, 1, size=(n_samples, 2))
        u[:, 0] += 1
        u[:, 1] -= 1
        c = np.random.uniform(-2, 2, size=(n_samples, 1))
        e_t = np.random.uniform(-1, 1, size=(n_samples, 1))
        e_y = np.random.uniform(-1, 1, size=(n_samples, 1))
        e_x = np.random.uniform(-1, 1, size=(n_samples, 2))
        coeffs1 = np.array([1, 1]).reshape(-1, 1)
        coeffs2 = np.array([-1, 2]).reshape(-1, 1)
...
def gen_scm(key: str) -> SCM:
    return {
        ...
        'backdoor_counterexample': backdoor_counterexample(),
        'backdoor_counterexample_binary': backdoor_counterexample_binary(),
        ...
        'nonlinear_padh_iv_modified': nonlinear_padh_iv_modified(),
        'linear_iv': linear_iv(),
        'binary_iv': binary_iv(),
        'binary_iv_mono': binary_iv_mono(),
